 ...
 * 결과: demo/brain.obj (양반구 + 주름 + 엽 돌출)
 */
import { writeFileSync } from 'fs';

const SEGS = 72, RINGS = 52;
const R = 1.0;
...
let obj = '# Brain mesh generated by handface\n';

// 양반구 각각 생성
for (const sign of [1, -1]) {
  obj += `g hemisphere_${sign > 0 ? 'right' : 'left'}\n`;
  const vOffset = sign > 0 ? 0 : (RINGS + 1) * (SEGS + 1);
  const verts = [];
  ...
  for (let j = 0; j < RINGS; j++) {
    for (let i = 0; i < SEGS; i++) {
      const a = vOffset + j * (SEGS + 1) + i + 1;  // OBJ는 1-based
      const b = a + 1;
      const c = a + SEGS + 1;
      const d = c + 1;
      obj += `f ${a} ${c} ${b}\n`;
...
console.log(`brain.obj generated: ${(obj.length / 1024).toFixed(0)} KB`);