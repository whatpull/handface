...
const SKIN_TRIS = [
  ...
  // 손가락 끝 연결
  7,8,11, 8,11,12, 11,12,15, 12,15,16, 15,16,19, 16,19,20,
];
const palmPositions = new Float32Array(SKIN_TRIS.length * 3);
...
function updateHandSkeleton(landmarks) {
  ...
  // 전체 피부 면 업데이트
  for (let i = 0; i < SKIN_TRIS.length; i++) {
    const li = SKIN_TRIS[i];
