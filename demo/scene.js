...
async function buildHippocampusFromToolbar() {
  ...
  document.getElementById('nf-bb-ltd')?.addEventListener('click', async () => {
    // V1 L4_E 첫 노드를 input 으로, IO 0번째를 error 로 demo.
    const v1l4 = (state.synapses || []).map(s => s.pre).filter(n => n && n.startsWith('v1_L4_E'));
    const inputs = v1l4.slice(0, 1);
    if (inputs.length === 0) return setBbStatus('⚠ input 노드 없음 (V1 L4_E 필요).');
...
function openSheet(kind) {
  ...
  if (kind === 'more')     { title.textContent = '더보기';          body.innerHTML = renderSheetMore(); }
...
async function enableCamera() {
  ...
  try {
    ...
    if (control.mediaStream && camPreview) {
      ...
      try { setCameraConnected(true); } catch (_) { /* noop */ }
      tryMountAsciiCamera();
      tryMountHand();
    }
  } catch (err) {
...
function disableCamera() {
  ...
  cameraNodeMounted = false;
  handNodeMounted = false;
  try { setCameraConnected(false); } catch (_) { /* noop */ }
}
...
window.addEventListener('snn-canvas:source-mounted', () => {
  // 카메라 미연결 상태 반영 (초기 dim, enableCamera 시 활성).
  try { setCameraConnected(cameraEnabled); } catch (_) { /* noop */ }
  tryMountAsciiCamera();
  tryMountHand();
