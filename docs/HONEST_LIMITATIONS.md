...
| Hand SNN (MediaPipe Hand) v6 mean-sub top-K + Jaccard | ⚠️ Phase 3.6 측정 — anatomically realistic appendFinger mock 4 gestures + sparse forced-disjoint top-K=5 → 30% accuracy. 원인: encoder.ts:237-256 자체 진단 "98% overlap" plain top-K trap. captured fixture (real MediaPipe via Playwright) iteration test: **v6 50% accuracy** (mean-subtracted top-K matching).  데이터: tests/integration/measurements/hand-snn-phase-3-production-measurement.json |
| Hand SNN (MediaPipe Hand) v7 cosine similarity matching | ✅ Phase 3.9 v7 (2026-06-03) — LiveSnn 이 stored cluster training features 와 cosine similarity 로 winner 결정 → worker-side Jaccard vigilance override. captured fixture iteration test **100% accuracy (8/8)**. threshold 0.97 (synthetic σ=0.02 jitter same-pose TP 98%, cross-pose FP 33% — synthetic stick figure thumb-only diff 본질 한계, real webcam 에서는 분리 예상). production deployed (commit 077a7b1). |
| Hand SNN v8 EMA cluster centroid update | ✅ Phase 3.9 v8 (2026-06-03) — cosine match 시 cluster training feature 를 EMA (α=0.1) update → 사용자 반복 표시 시 cluster 가 자세 분포로 수렴 → jitter robustness 향상. e2e test 검증: 10 EMA updates 후 cluster feature 가 새 sample 으로 명확 수렴 (diff 0.017 → 0.009). |
| Hand SNN v9 R-STDP reinforce on match | ✅ Phase 3.9 v9 (2026-06-03) — cosine match 시 reinforceAsync(cluster_id, 0.3) — SNN cluster weights 동기화 |
| Hand SNN v11 LiveSnn wrist-relative normalization | ✅ Phase 3.9 v11 (2026-06-03) — _normalizePatternV11 helper: pattern 의 wrist (indices 0,1,2) + middleMcp (27,28,29) 추출해 wrist-relative + palm-size normalized basis 로 변환. cross-pose discrimination 3.4x 향상 (1-cos sum 0.287→0.969). encoder 변경 없음 (R-STDP path 영향 0). 5가지 translation 범위 (origin, ±0.05, ±0.10, ±0.15, ±0.20) 에서 4/4 accuracy 검증. max diff after normalization = 0.000000 (perfect invariance). |
