...
| Hand SNN (MediaPipe Hand) synthetic mock 정확도 | ⚠️ Phase 3.6 측정 — anatomically realistic appendFinger mock 4 gestures + sparse forced-disjoint top-K=5 + production clusterTrainRStdp + Phase 2A fix chain (1st 30 / 2nd+ 90 trials) + 정직한 honest inference (cluster 영역 모름, 모든 features > 0.3 주입 + pattern-normalized firing rates) → **30% accuracy** (open_palm 60%, closed_fist 0%, thumbs_up 0%, peace_sign 60%; σ=0.05 noise). 직전 1-shot 25% 대비 marginal 5%p improvement 만. 원인: hand-spike-encoder.ts lines 237-256 자체 진단 — "4 gesture pairwise Jaccard distinctiveness 0.0185 (=98% overlap). R-STDP/ART/EWC 어떤 mechanism도 이 overlap 위에서는 분리 불가능". 직전 hand-snn-cluster-rstdp.json (2026-05-27) 100% 영역 oracle inference (applySparseTopK 영역 ground-truth 영역 사용) — production reality 영역 인정 불가. **결론: synthetic mock 본질적 indistinct → 실제 webcam 영역 인체 hand variation 측정만이 유효 검증.** 데이터: tests/integration/measurements/hand-snn-phase-3-production-measurement.json |
