...
// P214a Research Panel — 유사 패턴 스트레스 테스트.
//
// 5개 유사 4×4 패턴 (2×2 base + 1~2 픽셀 변형) 영역 학습 → P213 동일 지표
// 영역 측정. high-overlap 영역 selectivity 한계 영역 검증.

import { Fragment, useEffect, useState } from 'react';
import { runP214a, SIMILAR_PATTERNS, SIMILAR_NAMES } from '@/lib/research/p214a-similar';
import type { SelectivityMetrics } from '@/lib/research/p213-selectivity';

interface Progress { msg: string; pct: number }

export default function P214aPanel() {
  ...
  const run = async () => {
    ...
    setResults([]);
    try {
      const r = await runP214a((msg, pct) => setProgress({ msg, pct }));
      setResults(r);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      console.error('[P214a] experiment failed:', e);
      setError(msg);
    } finally {
      setRunning(false);
    }
  ...
  const downloadJson = () => {
    ...
    a.href = url;
    a.download = `p214a-similar-${Date.now()}.json`;
  ...
  return (
    <div className="rounded-lg border border-[#2a2a38] bg-[#18181f] p-6">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white">P214a — 유사 패턴 스트레스</h2>
          <p className="mt-1 text-xs text-[#8888aa]">
            5개 유사 4×4 패턴 (2×2 base + 1~2 픽셀 변형)을 학습하며 high-overlap selectivity를 측정합니다.
          </p>
        </div>
        <button
          type="button"
      ...
      <details className="rounded border border-[#2a2a38] bg-[#101015] p-3 text-xs text-[#aaa]">
        <summary className="cursor-pointer text-[#ccc]">유사 패턴 5종 (4×4)</summary>
        <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-5">
          {SIMILAR_PATTERNS.map((p, i) => (
            <div key={i} className="flex flex-col items-center gap-1">
              <PatternThumb pattern={p} />
              <span className="text-[10px] text-[#888]">{i + 1}. {SIMILAR_NAMES[i]}</span>
            </div>
          ))}
        </div>
      </details>
...
function MetricsTable({ results }: { results: SelectivityMetrics[] }) {
  return (
    <div>
      <h3 className="mb-2 text-sm font-semibold text-white">지표</h3>
...
function ConfusionMatrices({ results }: { results: SelectivityMetrics[] }) {
  return (
    <div>
      <h3 className="mb-2 text-sm font-semibold text-white">Confusion Matrix</h3>
      <p className="mb-3 text-xs text-[#8888aa]">
        rows = ground-truth, cols = mapped winner. 유사 패턴 영역 cluster collision 영역 비대각 셀 영역 표시.
      </p>
...
function ConfusionMatrixView({ metrics }: { metrics: SelectivityMetrics }) {
  ...
  return (
    <div className="rounded border border-[#2a2a38] bg-[#101015] p-3">
      ...
      <div className="grid gap-px" style={{ gridTemplateColumns: `auto repeat(${N}, 28px)` }}>
        <div />
