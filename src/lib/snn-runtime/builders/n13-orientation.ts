...
export function buildN13OrientationPreset(opts: N13PresetOptions = {}): N13PresetResult {
  ...
  // P215f revert (2026-05-20): 3.5 → 2.0 복원. P215f 가설 영역 partial cue
  // 약점 영역 OUT firing 영역 부족 영역 가정 영역 측정 영역 반증 — sparsity
  // 75% 영역 이미 충분 영역 fire, WTA selectivity 영역 약화 영역 root cause.
  // 3.5 영역 cluster 7 (winner-stuck attractor) 영역 self-reinforcing 영역
  // N=9 partial 67% → 22% 영역 -45%p 영역 regression. 학술 정합 불일치 —
  // strong recurrent excit 영역 winner stabilization 영역 단 'wrong winner'
  // 영역 lock-in 영역 hazard 영역 catch.
