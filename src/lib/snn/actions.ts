...
export function createActions(h: ActionHooks) {
  ...
  return {
    ...
    // 사용자 catch 2026-05-12: native confirm() → showDialog (accessible modal).
    // dialog callback-driven path — onConfirm 영역만 실제 reset 진행. 외부 호출자
    // 영역 fire-and-forget 정합 — Promise resolve 영역 dialog dismiss 즉시 (run()
    // wrapper 영역 busy state 영역 actual reset 영역 catch path 영역 dialog
    // confirm 시점 별도 run wrap).
    reset: () => {
      showDialog({
        kind: 'confirm',
        title: '회로 초기화 + LEARN 학습 데이터 폐기',
        message:
          '· 학습된 weight 모두 손실\n' +
