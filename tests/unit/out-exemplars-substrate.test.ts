...
const ORIENTATION_KEY = 'handface.out.exemplars.v1.orientation-6x6';
...
describe('out-exemplars — substrate isolation (Fix 1)', () => {
  it('S1: orientation incrementCount → orientation store 만 영향', () => {
    incrementCount('out_0_0', 'orientation-6x6', [1, 0, 0]);
    const o = loadExemplars('orientation-6x6');
  ...
  it('S3: setExemplarLabel — substrate isolation', () => {
    setExemplarLabel('out_2_0', 'orientation-6x6', 'horizontal');
  ...
  it('S4: subscribeExemplars — substrate-specific event delivery', () => {
    ...
    const offO = subscribeExemplars('orientation-6x6', (m) => {
    ...
    incrementCount('out_0_0', 'orientation-6x6');
  ...
  it('S6: legacy KEY migration — orientation 영역 default hydrate', () => {
    ...
    // 정직 한계: migration target 영역 out-exemplars.ts:50 'orientation' literal 영역
    // hardcoded — substrate upgrade 후 영역 legacy orientation store 영역 hydrate
    // 정합 (orientation-6x6 영역 fresh 영역 정합 — substrate-upgrade.md 영역 정합).
    const legacyData = {
