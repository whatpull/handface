...
const ORIENTATION_KEY = 'handface.out.exemplars.v1.orientation-5x5';
...
describe('out-exemplars — substrate isolation (Fix 1)', () => {
  it('S1: orientation incrementCount → orientation store 만 영향', () => {
    incrementCount('out_0_0', 'orientation-5x5', [1, 0, 0]);
    const o = loadExemplars('orientation-5x5');
  ...
  it('S3: setExemplarLabel — substrate isolation', () => {
    setExemplarLabel('out_2_0', 'orientation-5x5', 'horizontal');
  ...
  it('S4: subscribeExemplars — substrate-specific event delivery', () => {
    ...
    const offO = subscribeExemplars('orientation-5x5', (m) => {
    ...
    incrementCount('out_0_0', 'orientation-5x5');
  ...
  it('S6: legacy KEY migration — orientation 영역 default hydrate', () => {
    ...
    // 정합 (orientation-5x5 영역 fresh 영역 정합 — substrate-upgrade.md 영역 정합).
    const legacyData = {
